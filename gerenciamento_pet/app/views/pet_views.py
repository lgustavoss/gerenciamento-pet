...
from ..services import pets_service, cliente_service, consulta_service
    ...
    if request.method == "POST":
        ...
        if form_pet.is_valid():
            ...
            pet_novo = pet.Pet(dono=dono, nome=nome, nascimento=nascimento, categoria=categoria,
                               cor=cor)
            pets_service.cadastrar_pet(pet_novo)
...
def listar_pet_id(request, id):
    pet = pets_service.listar_pet_id(id)
...
def editar_pet(request, id):
    pet_antigo = pets_service.listar_pet_id(id)
    ...
    if form_pet.is_valid():
        ...
        pet_novo = pet.Pet(dono=dono, nome=nome, cor=cor, nascimento=nascimento,
                           categoria=categoria)
        pets_service.editar_pet(pet_antigo, pet_novo)
